...
typedef enum detect_eq_states { DEQInit, ON, WAIT           } detect_eq_states;
typedef enum detect_max_amp_states { DMAInit, M, Wait       } detect_max_amp_states;
...
unsigned char ping_led =0x00; //ping
unsigned char eq_led = 0x00; //detect earthquake
unsigned char max_led = 0x00; // max amplitude
unsigned char zero_led = 0x00; //zero-crossing
